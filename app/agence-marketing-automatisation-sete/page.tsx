...
const services = [
  {
    ...
    subtitle: 'DeepSignal — L\'écosystème de prospection',
    description: '4 modules interconnectés déclenchés par des signaux d\'intention réels : détection LinkedIn, cold email personnalisé par Claude AI, nurturing Brevo. Vous ne contactez que des prospects qui ont déjà levé la main.',
    points: ['10 à 60 RDV qualifiés/mois', 'Signaux LinkedIn, Email, SEO', 'Personnalisation par IA à l\'échelle', 'Scalable sans recruter'],
    featured: true,
  },
  {
    icon: '✦',
    badge: null,
    title: 'SEO automatisé par IA',
    subtitle: 'Auto-Blog SEO — Votre machine éditoriale',
    description: '3 workflows n8n couvrant l\'intégralité du cycle éditorial : détection d\'opportunités keywords, architecture Pillar-Cluster et publication automatisée. De 30 à 365 articles/mois à ~2€ l\'article.',
    points: ['DataForSEO + Claude AI + GPT-4o', 'Optimisation GEO (ChatGPT, Perplexity)', 'Vous pilotez la stratégie, l\'IA produit', 'Trafic organique sans y passer vos nuits'],
    featured: false,
  },
  {
    icon: '⚙',
            ...
            {services.map((svc, i) => (
              <FadeUp key={svc.title} delay={i * 0.1}>
                <div className={`relative flex flex-col h-full rounded-2xl p-8 transition-all duration-[250ms] hover:-translate-y-1.5 hover:shadow-[0_20px_40px_rgba(0,0,0,0.15)] ${
                  ...
                  <p className="font-body text-[14px] text-se-navy/50 mb-4">{svc.subtitle}</p>
                  <p className="font-body text-[15px] leading-[1.7] text-se-navy/60 mb-5 flex-1">{svc.description}</p>
